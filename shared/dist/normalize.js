...
export function l1(s) {
    return s.normalize("NFKC").toLowerCase().replace(/\s+/g, " ").trim();
}
export const BRACKETS = /[【[（(〔《「『][^】\]）)〕》」』]{0,30}[】\]）)〕》」』]/gu;
/**
...
export function l4(s) {
    ...
    t = t.replace(/[ー―‐−–—]/g, "");
    t = t.replace(/[ぁぃぅぇぉっゃゅょゎ]/g, (c) => String.fromCharCode(c.charCodeAt(0) + 1));
...
export function titleMatchKey(title) {
    return key(title, 5);
}
/** Maker match_key uses L1–L4 (level 4). */
export function makerMatchKey(maker) {
    if (!maker)
        return "";
    return key(maker, 4);
}
 ...
 */
export function assertNormalizationSelfCheck() {
    const a = "作品【演者A】";
    const b = "作品【演者B】";
    if (titleMatchKey(a) === titleMatchKey(b) || key(a, 5) === key(b, 5)) {
        throw new Error("normalization regression: distinct bracket content collapsed on production key path");
    }
}
