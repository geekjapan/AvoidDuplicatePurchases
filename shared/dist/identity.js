 ...
 */
export const LIBRARY_SOURCES = ["amazon", "ebookjapan", "kobo"];
/**
...
export const LIBRARY_SYNC_PROVIDERS = [
    {
        source: "amazon",
...
function hasOnlyPositiveQuery(url, name) {
 ...
 */
export function canonicalLibraryProductUrl(source, cid, value) {
    const url = parseCredentialFreeHttpsUrl(value);
    ...
    switch (source) {
        case "amazon": {
            if (url.hostname !== "www.amazon.co.jp")
                return null;
            const match = /^\/(?:dp|gp\/product|gp\/aw\/d)\/([A-Za-z0-9]{10})\/?$/i.exec(url.pathname);
        ...
        case "ebookjapan": {
            if (url.hostname !== "ebookjapan.yahoo.co.jp")
                return null;
        ...
        case "kobo": {
            if (url.hostname !== "books.rakuten.co.jp")
                return null;
...
export function isCanonicalLibraryProductUrl(source, cid, value) {
...
export function librarySyncProvider(source) {
