import { doujinLibraryUrl, doujinPageHasNext, } from "@adp/shared/adapters/fanza_doujin";
import { booksLibraryUrl, booksLibraryHasNext, booksContentsUrl, booksContentsHasNext, parseBooksLibraryPayload, } from "@adp/shared/adapters/fanza_books";
import { VIDEO_GRAPHQL_URL, videoPurchasedGraphqlBody, videoPageHasNext, } from "@adp/shared/adapters/fanza_video";
import { dlsoftLibraryUrl, dlsoftPageHasNext, parseDlsoftLibraryPayload, } from "@adp/shared/adapters/fanza_dlsoft";
import { importFanzaOnServer, markFanzaSyncedOnServer, } from "./server-api.js";
...
export async function runFanzaDoujinSync() {
    let page = 1;
    let inserted = 0;
    let updated = 0;
    let fetched = 0;
...
export async function runFanzaBooksSync() {
    let libPage = 1;
    const seriesQueue = [];
    let inserted = 0;
    let updated = 0;
    ...
    while (true) {
        ...
        if (!booksLibraryHasNext(libRes.data))
            break;
        libPage += 1;
    }
    for (const series of seriesQueue) {
        ...
        while (true) {
            ...
            inserted += imported.counts.inserted;
            updated += imported.counts.updated;
            if (!booksContentsHasNext(contentsRes.data))
                break;
            contentsPage += 1;
        }
    }
...
export async function runFanzaVideoSync() {
    let offset = 0;
    const limit = 100;
    let inserted = 0;
    let updated = 0;
...
export async function runFanzaDlsoftSync() {
    let page = 1;
    let inserted = 0;
    let updated = 0;
    let fetched = 0;
    ...
    while (true) {
        ...
        if (!dlsoftPageHasNext(res.data, totalFetchedItems))
            break;
        page += 1;
    }
...
export async function runAllFanzaSyncs() {
