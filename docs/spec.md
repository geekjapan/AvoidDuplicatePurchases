...
ステータス: **承認済み**(2026-08-03、Issue #8)。介入面の購入進行ブロックは **ADR-0001**(#57) で更新。
根拠となる決定: 地図 #1 の Decisions so far(#2〜#7, #9) および `docs/adr/0001-fail-closed-purchase-progression.md`。本書はそれらを統合し、残っていた未決事項を確定させる。実測の詳細は各プロトタイプの README を正とする。
...
- **購入進行ブロック(fail-closed)**: 確定重複があるあいだ、即購入・カートからの購入進行・カート後の購入進行面を止める(ADR-0001 / #57)。自動削除とオーバーライドは持たない。
...
**用語(ADR-0001 / domain glossary と同一)**:

- **確定重複**: lookup が同一サイト保有(`owned`)または他サイト完全一致(`other`)を返した状態。
- **重複候補**: `possible`(同一メーカー想定・dice ≥ 0.7)。購入済み表示にも購入進行ブロックにも使わない。
- **即購入**: 商品ページでカートを経由せず購入手続きへ進む操作。
- **購入進行面**: カートを離れたあと、支払完了前までの各ページ(内容確認・支払い手段・最終確定など)。サンクス/完了ページは対象外。
...
- 確定重複のとき **即購入のみ** を止める(CTA 無効化 + クリック横取り、理由表示)。
- **カート投入は許可**する。一覧からのカート投入も止めない。

### 一覧・検索ページ

サムネイル角に小さいオーバーレイバッジ(✓)。同一サイト `(source, cid)` 一致のみ(一覧からタイトル・メーカーを確実に取るのは店舗ごとに DOM が違い脆いため、横断照合は商品ページとカートに限定)。一覧の DOM フックは店舗ごとに実装時に確認(#5 の注意事項)。カート投入は止めない。

### カート

カートページを開いたらカート API(#4/#5 で確定、DLsite のみ DOM パース)で cid 一覧を取得し、サーバへ照合:

- **確定重複**(同一・他サイト購入済み)行に**警告バッジ + 「削除」ボタン**を表示。`possible` のみの行には出さない。
- 削除クリック → 削除 API(#9 の表)→ **トースト「削除しました — 元に戻す」を約 10 秒表示**。「元に戻す」= 復元 API。確認ダイアログは出さない。**自動削除はしない**。
...
- **カート全体ゲート**: 確定重複が **1 点でも** 残るあいだ、カートからの購入進行(購入手続きへ進む等)を止める。理由を表示する。解除はユーザーがカートから確定重複を消すか、保有ライブラリ上の事実を訂正することのみ。「それでも買う」オーバーライドは持たない。
- **実装フェーズの最初のタスクとして、安価な商品 1 件で削除→復元の実叩き確認を行う**(#9 で未実施。ユーザー立ち会いで)。

### 購入進行面(カート後・支払完了前)

介入対象 3 店舗で、カートを離れたあとの購入進行ページでも fail-closed を適用する。

- カートに確定重複が残っている前提の購入進行 CTA を止める(無効化 + クリック横取り、理由表示)。
- 判定は可能ならカート内容の再取得 + lookup。DLsite のようにカート一覧 API が無い面では、カートページで記録した確定重複状態を補助に使う。
- 支払完了後のサンクス等は対象外。ajax/API パスはページ介入しない。

### サーバ未起動時 / 照会不能

照会が失敗したら**警告も購入進行ブロックも出さない**(fail-open)。壊れたバッジや購入面の重いエラーバナーは出さない。ポップアップのアイコンバッジでのみ未接続を示す。確定重複が分からないときに日常購入を封じない(ADR-0001)。
